...
| The app shows "Remote environment unavailable … Session initialization failed (environment_deleted)" and the sessions moved to the "Other" section of the sidebar | Recoverable — deletion is a soft tombstone. `cd <the session's directory> && claude remote-control --session-id cse_…` re-registers the deleted environment id and reattaches the *original* thread (verified live: same URL and title back, the record's `last_init_error` cleared). It runs single-session; SIGTERM when done. What actually happens server-side: a *fresh* environment object is minted under the same id (its `created_at` is the resurrection time). Because the sidebar groups by id, every session of the old environment regroups under it. Some then re-bind to the live registration on their own the first time they're opened (observed for two sessions); others stay bound to the dead generation — the web client shows those as "Remote Control disconnected … turn Remote Control back on there", while the desktop app may keep showing a stale `environment_deleted` — until each is individually re-queued via `bridge/reconnect`. `--session-id` does that for its one session; reusing the environment from a pointer re-registers it but re-binds nothing. So recover the sessions you need one at a time: stop the unit (a running supervisor in the directory trips the split-brain guard), run `--session-id` for each from its own directory (recreate the directory, even empty, if it's gone — the session resumes by path), Ctrl-C once it shows Connected, delete the pointer each run leaves in that directory (project-dir names encode `_` as `-`), restart the unit. If you script it with `timeout`, either run it from a shell with no TTY or use `timeout --foreground` — plain `timeout` puts the child in a background process group, the interactive TUI then freezes on SIGTTIN, and every later signal (including timeout's own SIGTERM and your Ctrl-C) sits pending forever; only SIGKILL clears it. The reconnect endpoint is `POST /v1/environments/<env>/bridge/reconnect {"session_id": …}` with `anthropic-beta: environments-2025-11-01`, callable without stopping anything — untested from outside the CLI. |
