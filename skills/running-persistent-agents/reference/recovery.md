...
| The app shows "Remote environment unavailable … Session initialization failed (environment_deleted)" and the sessions moved to the "Other" section of the sidebar | The environment (the container the sidebar groups sessions under) was deleted server-side; deletion is final — there is no restore, and `--session-id` on such a session prints "Could not resume session — its environment has expired. Creating a fresh session instead." The app entries are husks; archive them. The sessions themselves are fully recoverable locally (next row). |
